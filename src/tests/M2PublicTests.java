...
public class M2PublicTests {
	...
	public void testEndTurnMethodLogic7() throws Exception {
		...
		g.getPlayer().getControlledArmies().add(army1); 
		g.endTurn(); 
		assertEquals("The Distance to target of all armies shouldn't be change if the ArmyWindow doesn't have a target",10,army1.getDistancetoTarget());
		...
		g.endTurn();
		assertEquals("The current location should be updated if the ArmyWindow reaches its target ","Rome",army1.getCurrentLocation());
		assertEquals("The status should be updated if the ArmyWindow reaches its target ",Status.IDLE,army1.getCurrentStatus());
		assertEquals("The targe value should be empty if the ArmyWindow reaches its target ","",army1.getTarget());
	...
	public void testEndTurnMethodLogic8() throws Exception {
		...
		{
			assertEquals("If the city is under siege, all ArmyWindow units should lose 10% ",54,u.getCurrentSoldierCount());
		}
	...
	public void testRelocateUnitArmyLogic() throws Exception {
		...
		assertTrue("The Parent ArmyWindow of the relocated unit should be changed to the corresponding ArmyWindow",new_army.equals(army));
	...
	public void testHandleAttackedUnitLogic() throws Exception {
		...
		assertTrue("Handle attacked unit shouldn't remove the unit from the ArmyWindow if its soldier's count greater than 0",army.getUnits().contains(archer));
		archer.setCurrentSoldierCount(0);
		army.handleAttackedUnit(archer);
		assertFalse("Handle attacked unit should remove the unit from the ArmyWindow if its soldier's count reaches 0",army.getUnits().contains(archer));
	...
	public void testConstructorCityConstructorInitialization()
		...
		if(city.getDefendingArmy().equals(null))
			fail("The defending ArmyWindow shouldn't be null");
		
		assertEquals("The defending ArmyWindow location should be inbside the city",city.getName(),city.getDefendingArmy().getCurrentLocation());
...
public void testPlayerInitiateArmyLogic1() throws Exception{
	...
	assertFalse("The Given unit should be removed from the defending ArmyWindow of the given city", city.getDefendingArmy().getUnits().contains(archerUnit1));
	}
@Test(timeout = 3000)
...
public void testPlayerInitiateArmyLogic3() throws Exception{
	...
	if(player.getControlledArmies().size() == 0)
		fail("Player controlled Armies should have the new ArmyWindow");
...
public void testPlayerInitiateArmyLogic4() throws Exception{
	...
	if(player.getControlledArmies().size() == 0)
		fail("Player controlled Armies should have the new ArmyWindow");
	assertTrue("The parent ArmyWindow of the given unit should be updated",player.getControlledArmies().get(0).equals(archerUnit1.getParentArmy()));
}
...
public void testPlayerLaySiegeLogic1() throws Exception{
	...
	assertEquals("The current status of the given ArmyWindow should be changed to BESIEGING", Status.BESIEGING,army.getCurrentStatus());
	}
	...
	private void testExistsInClass(Class aClass, String methodName, boolean implementedMethod, Class returnType,
		...
		private void playerRecruitUnitWithEnoughGoldHelper(Game game, String type, String cityName) throws Exception {
			...
			case "Archer":
				...
				assertTrue("A unit of type Archer should be added to the defending ArmyWindow.",
			...
			case "Cavalry":
				...
				unitAddedAndTreasuryDecrementedHelper(game, c, initialArmySize + 1, 5000, m);
				assertTrue("A unit of type Cavalry should be added to the defending ArmyWindow.",
		...
		private void unitAddedAndTreasuryDecrementedHelper(Game game, City c, int count, int treasury, MilitaryBuilding m) {
			assertEquals("A unit should be added to the defending ArmyWindow.", count, c.getDefendingArmy().getUnits().size());
			assertEquals("The recruited unit should have the defending ArmyWindow of the city as its parent ArmyWindow.", c.getDefendingArmy(),c.getDefendingArmy().getUnits().get(count-1).getParentArmy());
		...
		private void unitNotAddedAndTreasuryNotDecrementedHelper(Game game, int expectedArmyCount, int treasury, City c) {
			assertEquals("A unit should not be added to the defending ArmyWindow.", expectedArmyCount,
		...
		private void unitAttack(Unit attackUnit, double archerFactor, double infantryFactor, double cavalryFactor ) {
			...
			if (attackUnit.getCurrentSoldierCount() * archerFactor >= archerCurrentSoldierCountBefore){
				...
				assertFalse("The Target should be removed from its parent ArmyWindow's unit by calling handle attacked units method",target_Army.getUnits().contains(archerTarget));
			...
			if (attackUnit.getCurrentSoldierCount() * cavalryFactor >= cavalryCurrentSoldierCountBefore){
				...
				assertFalse("The Target should be removed from its parent ArmyWindow's unit by calling handle attacked units method",target_Army.getUnits().contains(cavalryTarget));
			...
			if (attackUnit.getCurrentSoldierCount() * infantryFactor >= infantryCurrentSoldierCountBefore){
				assertEquals(
						"The Target Current Soldier Count should be 0 but was " + infantryTraget.getCurrentSoldierCount(),
						infantryTraget.getCurrentSoldierCount(), 0);
				assertFalse("The Target should be removed from its parent ArmyWindow's unit by calling handle attacked units method",target_Army.getUnits().contains(infantryTraget));
			}
			else
				assertEquals(
						"The Target Current Soldier Count should be "
